...
// WithIsPartialRequest returns a copy of parent in which the is partial object metadata request value is set
func WithIsPartialRequest(parent context.Context, isPartialReq bool) context.Context {
	return WithValue(parent, ProxyPartialObjectMetadataRequest, isPartialReq)
}

// IsPartialRequestFrom returns the value of the is partial object metadata request key on the ctx
func IsPartialRequestFrom(ctx context.Context) (bool, bool) {
	info, ok := ctx.Value(ProxyPartialObjectMetadataRequest).(bool)
	return info, ok
}
...
func WriteObject(statusCode int, obj runtime.Object, w http.ResponseWriter, req *http.Request) error {
	...
	if info, ok := apirequest.RequestInfoFrom(ctx); ok {
		...
		isPartialReq, _ := IsPartialRequestFrom(ctx)
		if isPartialReq {
			convertGvk, _ := ConvertGVKFrom(ctx)
			if convertGvk != nil {
