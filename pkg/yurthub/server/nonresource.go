...
func wrapNonResourceHandler(proxyHandler http.Handler, config *config.YurtHubConfiguration, rest *rest.RestConfigManager) http.Handler {
	wrapMux := mux.NewRouter()
	// register handler for non resource requests
	for i := range nonResourceReqPaths {
		wrapMux.Handle(nonResourceReqPaths[i], withNonResourceRequest(nonResourceReqPaths[i], config.StorageWrapper, rest)).Methods("GET")
	}
...
func withNonResourceRequest(path string, sw cachemanager.StorageWrapper, restMgr *rest.RestConfigManager) http.Handler {
